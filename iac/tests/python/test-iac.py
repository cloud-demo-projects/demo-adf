...
class TestModule(unittest.TestCase):
    ...
    def test_template_files(self):
        bicep_files = glob.glob('iac/bicep/DataFactory/*.bicep'.format(os.environ['BUILD_REPOSITORY_NAME']))
        self.assertGreater(len(bicep_files), 0, 'There should be at least one bicep file in the template folder.')

        bicep_config_path = 'iac/bicep/DataFactory/bicepconfig.json'.format(os.environ['BUILD_REPOSITORY_NAME'])
